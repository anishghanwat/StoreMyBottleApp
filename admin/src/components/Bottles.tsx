...
export function Bottles() {
  ...
  const handleSave = async () => {
    ...
    if (formData.brand.trim().length > 255) {
      toast.error("Brand name must be less than 255 characters")
      return
    }
    ...
    if (formData.name.trim().length > 255) {
      toast.error("Bottle name must be less than 255 characters")
      return
    }
    ...
    if (price > 999999.99) {
      toast.error("Price must be less than 999,999.99")
      return
    }
    if (!formData.ml || formData.ml.trim().length === 0) {
      toast.error("Please enter volume in ml")
      return
    }
    const volume = parseInt(formData.ml)
